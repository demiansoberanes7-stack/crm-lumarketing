...
/* ============================================================
 * Chatbot — conversaciones persistentes del chat IA interno
 * ============================================================ */

export const chatbotConversation = pgTable(
  ...
  (t) => [
    index("chatbot_conv_org_user_idx").on(t.organizationId, t.userId),
  ]
);
...
export const chatbotMessage = pgTable(
  "chatbot_message",
  {
    ...
    conversationId: varchar("conversation_id", { length: 255 })
      .notNull()
      .references(() => chatbotConversation.id, { onDelete: "cascade" }),
    role: varchar("role", { length: 20 }).notNull(),
    content: text("content").notNull(),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  },
  (t) => [
    index("chatbot_msg_conv_idx").on(t.conversationId, t.createdAt),
  ]
);
