...
export function WahaAdvanced({ initial, engine, busy, onSave }: { initial: WahaSettings | null; engine: string | null; busy: boolean; onSave: (settings: WahaSettings) => Promise<void> }) {
  ...
  return <details className="rounded border p-4"><summary className="cursor-pointer font-semibold">Configuración avanzada de sesión</summary>
    <form className="mt-4 space-y-3" onSubmit={(e) => { e.preventDefault(); void onSave(settings).then(() => set("proxyPassword", "")); }}>
