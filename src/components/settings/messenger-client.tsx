...
const HELP: Record<Source, { title: string; items: string[] }> = {
  zernio: {
    ...
    items: [
      "La página se vincula en el panel de Zernio, no desde LUMARK. Copia de ahí el accountId de la cuenta de Facebook conectada.",
      "La API key se crea en Zernio → Settings → API Keys y se muestra una sola vez (empieza con sk_).",
      "El mismo webhook de Zernio entrega Instagram, WhatsApp y X si esas cuentas están conectadas; LUMARK filtra por plataforma y solo ingiere lo de Facebook aquí.",
        ...
        <CardContent className="space-y-4">
          ...
          {error && (
            <div className="flex items-start gap-2 rounded-lg border border-destructive bg-destructive/10 p-3 text-sm text-destructive">
              ...
              <div>
                <p className="font-medium">Error al conectar Messenger</p>
